...
export const {
  ...
  callbacks: {
    async jwt({ token, user }) {
      // If user is signing in, add user information to the token
      if (user) {
        token.id = user.id;
    ...
    async session({ session, token }) {
      // Add token information to the session object
      if (token) {
        session.id = token.id;
    ...
    async redirect({ url, baseUrl }) {
      // Ensure redirects stay within your app
      return url.startsWith(baseUrl) ? url : baseUrl;
    },
  },
  debug: true, // Enable to get logs in the console for debugging in production
});
